...
class SubscribeMessage {
    ...
    convenience init(dict: Dictionary<String,Any>) {
        self.init()
        if let topic = dict[Keys.TOPIC] as? String { self.topic = topic }
        if let unsubscribe = dict[Keys.UNSUBSCRIBE] as? Bool { self.unsubscribe = unsubscribe }
        if let eventValue = dict[Keys.EVENTVALUE] as? String { self.eventValue = eventValue }
    }
}
...
func returnPermissionResult(isGranted: Bool){
    DispatchQueue.main.async(execute: {
        if (isGranted){
            Matchifyer.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('push-permission-request', { detail: 'granted' }))")
        } else {
            Matchifyer.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('push-permission-request', { detail: 'denied' }))")
        }
    })
}
...
func returnPermissionState(state: String){
    DispatchQueue.main.async(execute: {
        Matchifyer.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('push-permission-state', { detail: '\(state)' }))")
    })
}
        ...
        case .notDetermined:
            ...
            UNUserNotificationCenter.current().requestAuthorization(options: authOptions, completionHandler: { (success, error) in
                if error == nil {
                    ...
                    } else { returnPermissionResult(isGranted: false) }
                } else { returnPermissionResult(isGranted: false) }
            })
        case .denied: returnPermissionResult(isGranted: false)
        case .authorized, .ephemeral, .provisional: returnPermissionResult(isGranted: true)
...
func handlePushState() {
    UNUserNotificationCenter.current().getNotificationSettings () { settings in
        ...
        @unknown default: returnPermissionState(state: "unknown"); return;
        }
    }
}

func checkViewAndEvaluate(event: String, detail: String) {
    if (!Matchifyer.webView.isHidden && !Matchifyer.webView.isLoading ) {
        DispatchQueue.main.async(execute: {
            Matchifyer.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('\(event)', { detail: \(detail) }))")
        })
    } else {
...
func sendPushToWebView(userInfo: [AnyHashable: Any]){
    ...
    } catch { print("ERROR: userInfo parsing problem"); return }
